...
[optional]
-a, --automodel
	The simplest method for simple comparitive modeling. Will not give great results but suggested when many chain breaks are present. [default: True]

-f, --fixed_automodel
	Builds an automodel and keeps the non-missing residues fixed, whereas they can move in the other methods. [default: False]
	]
-l, --loopmodel 
	Builds a model by refining the loop with the missing residues. Suggested when have one small chain break in the PDB. [default: False]
...
with open('./'+ pdb_id + '.pdb') as pdb_file:
	pdb_file_data = pdb_file.readlines()

with open('./alignment.ali') as aln_file:
...
def usage():
	Print("Please see headers of python scripts for details of proper usage")
...
def find_missing_residues(pdb_seq):
	...
	chain_labels = ['A','B','C','D','E','F','G','H','I','K','L','M']
	...
	for pdb_seq in chains_l:
		...
		for i in range(len(pdb_seq)):
			if (pdb_seq[i] == '-'):
				missing_res_l.append(res_idx+1)
			res_idx = res_idx + 1
